...
#knitr::opts_knit$set(root.dir = "~/diss/CORIANDER/r.script.coriander/output/120517/")
...
cytobands = read.table("../../tables/cytobands.tsv", header=TRUE, row.names = 1, stringsAsFactors = F)
...
stats[nrow(stats) +1, ] = list(as.character("unique_mapped_reads"), sum(patient[ , 6]))
stats[nrow(stats) +1, ] = list(as.character("patient ID"), patient_name)
stats[nrow(stats) +1, ] = list(as.character("gender"), gender)

library(knitr)
kable(stats, digits = 0, col.names = NULL, align = "l")
```
...
region_merge = function(bin_data, regions, sd_factor) {
  ...
  if(nrow(regions) < 1) return(regions_merged)
...
plot_bins_regions_ideogram <- function(bin_data, cytobands, region_merged,
  ...
  plot_patient <- qplot(x, y, ylim=c(-50, 750)) +
    ...
    #das eigentliche ideogram
                  geom_rect(data = cytobands[cytobands$Chr == chrn, ],
                    aes(NULL, NULL, xmin = Start, xmax = End, fill = Band.type),
                        ymin = -50, ymax = -10, color = "gray", size = 0.5, alpha = 0.4) +
                  scale_fill_manual(values=c("acen" = "#2b8cbe", "gneg" = "#f7f7f7", "gpos100" = "#252525",
                                             "gpos25" = "#cccccc", "gpos50" = "#969696",
                                             "gpos75" = "#636363", "gvar" = "#045a8d", "stalk" = "#be8c2b",
                                              "add" = "blue", "del" = "red")) +
    ...
    # die regione einzeichnen
                  geom_rect(data = region_merged[region_merged$Chr == chrn,],
                            aes(NULL, NULL, xmin = Start, xmax = End, fill = Direction),
                                #ymin = median(patient[patient$Chr == chrn, 6]),
                                #ymax = median(pon[pon$Chr == chrn, "mean"]),
                                ymin = 250,
                                ymax = 500,
...
plot_genome_wide_cnv = function(bin_data) {
  ...
  plot_all_chromosomes <- qplot(global_Start, pat_norm, 
    ...
    geom_point(data = bin_data[bin_data$aberration == "add", ], 
               aes(x = bin_data[bin_data$aberration == "add", "global_Start"], 
                   y = bin_data[bin_data$aberration == "add", "pat_norm"], color = "blue")) +
    geom_point(data = bin_data[bin_data$aberration == "del", ], 
               aes(x = bin_data[bin_data$aberration == "del", "global_Start"], 
                   y = bin_data[bin_data$aberration == "del", "pat_norm"], color = "orange")) +
...
## Genes from AMl Guidline 2016 or cancer driver genes paper:
```{r echo = FALSE}
