...
if(tolower(meta$gender) %in% c("f", "female")) {
  ...
  ponXf$Chr = "chrX"
  pon = rbind(pon[!pon$Chr %in% c("chrX_F", "chrX_M", "chrY_M"), ], ponXf)
  ...
  ponYm$Chr = "chrY"
  pon = rbind(pon[!pon$Chr %in% c("chrX_F", "chrX_M", "chrY_M"), ], ponXm, ponYm)
  pon$patient = patient[, 6]
}

pon$patient.norm = pon$patient / median(pon$patient) *2
...
with(pon[!(pon$gc_blocked | pon$var_blocked), ], {
  ...
  pon.hist = hist(sum/median(sum)*median(patient), breaks=seq(0, max(sum), by=50), plot=F)
...
hist(pon$rate[!(pon$gc_blocked | pon$var_blocked)])
pon$binom = pon$binom = apply(pon[, c("patient", "sum")], 1, function(x) ifelse(x[2] > 0, binom.test(x[1], x[2]+x[1], median(pon$rate[!(pon$gc_blocked | pon$var_blocked | pon$rate == 0)]))$p.value, 1))
pon$binom.adjust = p.adjust(pon$binom, method="hochberg")

plot(pon$sum/median(pon$sum), pon$patient/median(pon$patient), xlim=c(0, 2), ylim=c(0, 2), col=ifelse(pon$Chr %in% c("chrX", "chrY"), "#ff111122", "#11111122"), pch=16)
abline(0, 1)

plot(pon$bin, pon$patient.norm, ylim=c(0,4))
pon$norm.sum = pon$sum/median(pon$sum)
pon$norm.sd = pon$norm_sd/2
pon.norm.sum.reg = glm(norm.sd ~ norm.sum, pon, family=binomial)
predict(pon.norm.sum.reg, newdata=data.frame(norm.sum=seq(0, 2, 0.01)))
abline(pon.norm.sum.reg)
```
