...
```{r echo = FALSE}
hist(pon$patient, breaks=seq(0, max(pon$patient)+20, by=20),
     xlim=c(0, median(pon$patient)*2),
     freq=F, xlab="Depth per bin", border=F, col="blue", main="")
pon.hist = hist(pon$sum/median(pon$sum)*median(pon$patient), breaks=seq(0, max(pon$sum), by=50), plot=F)
lines(pon.hist$mids, pon.hist$density, col="red")
abline(v = c(0.5, 1, 1.5, 2) *median(pon$patient), col='gray', lty=2)
legend("right", legend="scaled PON", col="red", lty=1, cex=0.8)
mtext(c("x1", "x2", "x3", "x4"), at=c(0.5, 1, 1.5, 2) *median(pon$patient), side=3)
