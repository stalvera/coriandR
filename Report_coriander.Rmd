...
patient = read.table(patient.meta$count_data, header=TRUE, row.names = 1, stringsAsFactors = F)
...
## Sample ID
```{r echo = FALSE}
cat(patient.meta$name)
```

```{r echo = FALSE}
stats = data.frame(t(read.table(patient.meta$mapping_stats, stringsAsFactors = F, row.names=1)))
stats$unique_mapping_pairs = round(sum(patient[,6]/2))
```

| Sample characterisic | Value |
|---------------------:|:------|
| Sample name          | `r patient.meta$name` |
...
with(pon[!pon$blocked, ], {
  ...
  pon.hist = hist(norm.clean.mean*median(patient), breaks=seq(0, max(norm.clean.mean*median(patient)*2, na.rm=T), by=20), plot=F)
...
hist(pon$rate[!pon$blocked], breaks=100)
...
with(pon[!pon$blocked, ],
     plot(bin, patient.norm*ploidy, ylim=c(0, 4),
          col = ifelse(gauss.adjust < 0.01, "#ff000055", "#11111111"),
          pch = 16, xaxt="n", xlab="", las = 1, ylab="ploidy", xaxs="i")
)
with(pon[!pon$blocked & pon$gauss.adjust < 0.01 , ],
     arrows(bin, norm.clean.mean*ploidy, bin, patient.norm*ploidy, length=0.03, angle=45, code=2, col="#ff000022", lwd=3))
abline(v = c(pon$bin[pon$Start == 1], nrow(pon)), col="#55555555", cex = 0.5)
with(contigs[(1:(nrow(contigs)/2))*2, ], axis(side=1, labels=chr, at=pos, line=-.5, cex.axis=0.5, tick=F))
with(contigs[(1:(nrow(contigs)/2))*2-1, ], axis(side=1, labels=chr, at=pos, line=-1, cex.axis=0.5, tick=F))
```
...
N = sum(pon$patient[!pon$blocked])
pon$pbbinom = 1
pon$pbbinom[!pon$blocked] = sapply(which(!pon$blocked), function(x) bbinom.pvalue(pon$patient[x], N, exp(pon$alpha[x]), exp(pon$beta[x])))

plot(pon$bin, pon$patient,
     col =ifelse(pon$pbbinom < 0.01, "#ff1111aa", "#11111199"), pch = ".", yaxs="i",
