...
```{r}
logAlphaStart = log(colMeans(Mm[ ,sel]))
```
...
```{r}
minLoss = as.numeric(loss(model, Mm[, sel]))
```
...
```{r}
cat("Initial loss: ", as.numeric(loss(model, Mm[, sel])), "\n")
  
for (i in 1:2000) {
  grads = grad(model, Mm[, sel])
...
```{r}
alpha = rep(NA, NCOL(Mm))
alpha[sel] = as.array(minPars[[1]])
```
