...
pon.norm = data.frame(pon.norm/apply(pon.norm, 1, median, na.rm=T))

plot(rep(pon$bin, nrow(meta)), c(as.matrix(pon.norm)*pon$ploidy),      #col=rep(gray(1:length(pon.cols)/length(pon.cols), alpha=0.1), each=nrow(pon)),
...
learn = function(data, iterations=1000, rate=0.01, verbose=FALSE) {

  logAlphaStart = log(colMeans(data))
  ...
  optimizer = optimizer_adam(lr = rate, amsgrad = TRUE)
  minLoss = as.numeric(loss(model, data))
  minPars = model$trainable_variables
  
  if(verbose) { cat("Initial loss: ", minLoss, "\n") }
  for (i in 1:iterations) {
    grads = grad(model, data)
    ...
    if (verbose & i %% 20 == 0)
      cat("Loss at step ", i, ": ", minLoss, "\n")
  }
  return(as.array(minPars[[1]]))
}

pon$alpha = NA

sel = !pon$blocked & pon$gender == "neutral"
Mm = t(pon[sel, meta$pon.col])
pon$alpha[sel] = learn(Mm, 1000, verbose=T)

sel = !pon$blocked & pon$gender != "M"
temp_gender = pon$gender[sel]
Mm = t(pon[sel, meta$pon.col][, meta$gender == "F"])
falpha = learn(Mm, 1000, verbose=T)
pon$alpha[sel & pon$gender == "F"] = falpha[temp_gender == "F"]

sel = !pon$blocked & pon$gender != "F"
temp_gender = pon$gender[sel]
Mm = t(pon[sel, meta$pon.col][, meta$gender == "M"])
malpha = learn(Mm, 1000, verbose=T)
pon$alpha[sel & pon$gender == "M"] = malpha[temp_gender == "M"]
...
pon$beta = NA
pon$beta = sapply(pon$bin, function(x) logSumExp(pon$alpha[-x]))
