...
plot(rep(gc, ncol(pon.norm)), c(as.matrix(pon.norm.clean[!pon$gc_blocked & !pon$var_blocked, ])*pon$ploidy), pch=20, xlim=c(0.25, 0.6), ylim=c(0, 4),
...
plot(rep(pon$bin[!pon$gc_blocked & !pon$var_blocked], nrow(meta)), c(as.matrix(pon.norm.clean[!pon$gc_blocked & !pon$var_blocked, ])*pon$ploidy[!pon$gc_blocked & !pon$var_blocked]),
     col ="#11111105", pch = 16, ylim=c(0, 4), yaxs="i",
     xaxt="n", xlab="Chromosomes", las = 1, ylab="Ploidy", xaxs="i")
abline(v = c(pon$bin[pon$Start == 1], nrow(pon)), col="#55555555", cex = 0.5)
with(contigs[(1:(nrow(contigs)/2))*2, ], axis(side=1, labels=chr, at=pos, line=1, cex=0.5, tick=F))
