---
title: "Report to create a new custom Panel of normals (PON) with coriandR"
date: "`r format(Sys.time(), '%d-%m-%Y')`"
output: pdf_document
---

> In this markdown script a new PON is created from the fastq files and a gender table. To create the gender table, see the examples in the documentation.

> The data adjustment with applied statistical methods is presented in this report step by step.

```{r, include=FALSE}
#pfad zum ordner, falls skript manuell ausgefuehrt wird
knitr::opts_knit$set(root.dir = "/media/vera/big_data/mr_genoms/pon.marburg.compliete")
library(knitr)
...
# data definition
pon.data = read.table("/media/vera/big_data/mr_genoms/pon.marburg.compliete/pon.data.tsv", stringsAsFactors = F)
pon.name = pon.data[1,1]
fastq.dir = pon.data[1,2]
gender.dir = pon.data[1,3]

# gc-gehalt, erstellt mit bedtools
gc = read.table("/media/vera/big_data/mr_genoms/pon.marburg.compliete/GRCh38.p13.genome.1M.nucl", stringsAsFactors = T)

pon = read.table("/media/vera/big_data/mr_genoms/pon.marburg.compliete/pon.fc.tsv", header = TRUE, row.names = 1, stringsAsFactors = T)
gender_pon = read.table(gender.dir, header = TRUE, sep = ",", row.names = 1)
pon.cols = grep("bam", names(pon))  # which columns have the actual data
...
## PON name
```{r, PON name, echo = FALSE}
print(pon.name)
```

# Sequencing depth of individual PON subjects
> On a log-Scale we see that the PON data is similarly distributed at different sequencing depths.

> In this graphic, individual PON subjects are represented with their genders. It is a logarithmic representation of the sequencing depth.

> The subjects should have roughly the same sequencing depth. The male subjects have two point clouds (the first for the single X chromosome and the second for the double somatic chromosomes).

```{r, Reading in the PON and Patient counts, warning=F, fig.height=12, echo = FALSE}
# Reading in the PON and Patient counts

pon$bin = 1:nrow(pon)  # plain number all bins
contigs = t(tapply(pon$bin, pon$Chr, mean)) # middle coordinates of chromosomes for axis drawing

plot(c(t(pon[ ,pon.cols])), rep(1:length(pon.cols), nrow(pon)),
     log = "x", xlab="Read depth in log10", ylab = "Panel of Normals Subjects", yaxt = "n",
     pch = ".")
# col = hsv(1:length(pon.cols)/ length(pon.cols), alpha = 0.1),

axis(4, 1:length(pon.cols), las = 1, gender_pon$sex)
```
...
pon$sum = rowSums(pon[, pon.cols], na.rm=T)


plot(pon$bin, pon$sum, ylim = c(0, qnorm(0.999, mean(pon$sum), sd(pon$sum))),
     col = ifelse(pon$sum < qnorm(0.01, mean = mean(pon$sum), sd=sd(pon$sum)) | pon$sum > qnorm(0.99, mean = mean(pon$sum), sd = sd(pon$sum), lower.tail=T), "tomato3", "slategray"),
     pch = 20, xaxt="n", xlab="Chromosomes", las = 1, ylab="Read depth", frame=F)
abline(v = c(pon$bin[pon$Start == 1], nrow(pon)), col="#55555555", cex = 0.5)
axis(1, contigs, colnames(contigs), cex = 0.5)
...
\newpage
# Normalizing PON and patient by library median
...
> Plotted over all chromosomes, normalized to the ploidy of 2, the visual interpretation becomes very straight forward (some noise might be problematic). We have some unsightly spikes in PON, of bins with either much more or much less counts than average.

> As the filtering progresses, the noise around the baseline will decrease. 

> The male subjects have only one copy of X and Y chromosome. 

```{r, Plotted over all chromosomes, echo = FALSE, fig.width = 12, fig.height = 5}
#pon$norm.pat = pon$pat/median(pon$pat, na.rm=T)*2

plot(rep(pon$bin, length(pon.cols)), c(as.matrix(pon.norm)), ylim=c(0, 6),      #col=rep(gray(1:length(pon.cols)/length(pon.cols), alpha=0.1), each=nrow(pon)),
     col ="slategray", pch = 16, xaxt="n", xlab="Chromosomes", las = 1, ylab="Read depth", frame=F)
abline(v=c(pon$bin[pon$Start == 1], nrow(pon)), col="#55555555")
axis(1, contigs, colnames(contigs), cex = 0.5)
#lines(pon$bin, pon$norm.pat, col="blue")
```
...
```{r, compute.GC-content, echo = FALSE}
# gc gehalt muss auch gegendert werden

gendering_gc <- function(gc) {
  
  # zuerst das X-Chromosom in chrX_F und chrX_M aufteilen
  chrn = "chrX"
  # in eine separate tabelle werden alle zeilen von x-chr reingepackt
  gc.copy = gc[gc$V1 == chrn, ]
  # allen zeilen wird der suffix "_M" zugewiesen
  gc.copy$V1 = paste0(gc.copy$V1, "_M")
  gc.new = rbind(gc, gc.copy)
  
  gc.copy = gc[gc$V1 == chrn, ]
  gc.copy$V1 = paste0(gc.copy$V1, "_F")
  gc.new = rbind(gc.new, gc.copy)
  ...
  # zeilen mit den eigentlichen X- und Y-chromosomen loeschen
  gc.new = gc.new[!(gc.new$V1 == "chrX" | gc.new$V1 == "chrY"), ]
  gc = gc.new
}
...
plot(rep(pon$gc, ncol(pon.norm)), c(as.matrix(pon.norm)), pch=20, xlim=c(0.2, 0.7), ylim=c(0, 4),
     col = "steelblue4",
...
plot(pon$gc, pon$norm.var, pch=20, ylim=c(0.0001, 0.3), xlim=c(0.2, 0.6),
     col=ifelse(pon$gc_blocked, "tomato3", "slategray"),
     xlab="GC-content", ylab="Variance of normalized abundance", las = 1, cex = 0.5)
gc.var = predict(loess(norm.var ~ gc, pon[pon$gc > 0.35 & pon$gc < 0.6,]), 350:600/1000)
lines(350:600/1000, gc.var)
#library(robust)
...
plot(rep(pon$bin, length(pon.cols)), c(as.matrix(pon.norm)), ylim=c(0, 6),
     col = ifelse(pon$gc_blocked, "tomato3", "slategray"), pch=20,
     xaxt="n", xlab="Chromosomes", las = 1, ylab="Read depth")
...
> The bins that were excluded from the statistical calculations are shown in red: red cross - because of GC content, red point - because of variance.
...
plot(rep(pon$bin, length(pon.cols)), c(as.matrix(pon.norm.clean)), ylim=c(0, 6),
     col=ifelse(pon$var_blocked, "tomato3", "slategray"), pch=ifelse(pon$gc_blocked, 4, 20),
     xaxt="n", xlab="Chromosomes", las = 1, ylab="Read depth")
abline(v=c(pon$bin[pon$Start == 1], nrow(pon)))
axis(1, contigs, colnames(contigs), cex = 0.5)
```
...
plot(pon$gc, pon$norm.clean.var, col="yellowgreen", pch=19, ylim=c(0.00001, 0.3), xlim=c(0.3, 0.6), xlab="GC-content", ylab="Variance of normalized abundance", las = 1, cex = 0.5)
...
plot(rep(pon$gc, ncol(pon.norm)), c(as.matrix(pon.norm.clean)), pch=20, xlim=c(0.2, 0.7), ylim=c(0, 4),
     col=gray(1:length(pon.cols)/length(pon.cols), alpha=0.1), xlab="GC-content",
...
```{r, PON normalisation, echo = FALSE}
