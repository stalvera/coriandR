...
meta$pon.col = unlist(sapply(meta$sample, function(x) grep(gsub("-", ".", x), names(pon))))
...
plot(pon$bin, pon$sum, ylim = c(0, qnorm(0.999, mean(pon$sum), sd(pon$sum))),
     ...
     pch = 16, xaxt="n", xlab="", las = 1, ylab="Read depth")
...
pon.norm = data.frame(t(t(pon.norm)/(apply(pon.norm, 2, sum, na.rm=T)/1000000)))
...
plot(rep(pon$bin, length(pon.cols)), c(as.matrix(pon.norm)),
...
plot(rep(pon$bin, length(pon.cols)), c(as.matrix(pon.norm.clean)),
     col=ifelse(pon$var_blocked, "tomato3", "#11111105"), pch=ifelse(pon$gc_blocked, 4, 16),
     ylim=c(0, 4), yaxs="i",
     xaxt="n", xlab="Chromosomes", las = 1, ylab="Ploidy", xaxs="i")
abline(v = c(pon$bin[pon$Start == 1], nrow(pon)), col="#55555555", cex = 0.5)
with(contigs[(1:(nrow(contigs)/2))*2, ], axis(side=1, labels=chr, at=pos, line=1, cex=0.5, tick=F))
...
plot(pon$gc, pon$norm.clean.var, col="#44991105", pch=19, ylim=c(0.00001, 0.3), xlim=c(0.2, 0.7), xlab="GC-content", ylab="Variance of normalized abundance", las = 1, cex = 0.5)
