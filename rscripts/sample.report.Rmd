...
This report documents the calculated karyotyping and estimation of CNVs from sequencing data of hamatological malignances and solid tumours in research with `coriandR`. 
...
# Here, cancer driver genes (Bailey et al. 2018) and genes that play an important role in disease development of Acute Myeloid Leukemia (Papaemmanuil et al. 2016) are used for estimation of CNVs in sample with haematological malignances
...
```{r abberations in bins, echo = FALSE}
# Definition of abberant bins in tumour sample
# A deletion in a bin is detected if the normalized value for that bin is below the median for 
# all normalized PON samples with a significance level of $\alpha$ = 0.05. 
# An amplification leads to a value above the median.
