...
with(pon[!pon$blocked, ],
     plot(bin, ifelse(patient.norm > 2, 4, patient.norm*2), ylim=c(0, 4),
          ...
          pch = ifelse(patient.norm > 2, 17, 20), xaxt="n", xlab="", las = 1, ylab="Ploidy", xaxs="i")
)
...
for(i in unique(pon$Chr)) {
  with(pon[pon$Chr == i,], {
    plot(Start, ifelse(patient.norm > 2, 4, patient.norm*2), col = ifelse(blocked, 0, c("black", "blue", "red")[aberration]), ylim=c(0,4), xlim=c(0,max(End)), xaxt='n', xaxs="i", xlab='', ylab='Ploidy', pch=ifelse(patient.norm > 2, 17, 20), las=2)
