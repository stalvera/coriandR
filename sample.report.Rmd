...
# Read PON table
pon = read.table("pon.tsv", header=TRUE, row.names = 1, stringsAsFactors = F)
...
pon$patient.norm = pon$patient / median(pon$patient) * 2 / pon$ploidy
...
stats = data.frame(t(read.table("mapping.stats.tsv", stringsAsFactors = F, row.names=1)))
...
with(pon[!pon$blocked, ], 
     {
      ...
      pon.hist = hist(norm.clean.mean*median(patient), breaks=seq(0, max(norm.clean.mean*median(patient)*2,  na.rm=T), by=20), plot=F, xlim = 4* median(patient))
...
\newpage
```{r binomial test with p adjustment, echo = FALSE, fig.width=7, fig.height=3}
pon$rate = with(pon, ifelse(sum > 0, patient/(sum+patient), 0))
#hist(pon$rate[!pon$blocked], breaks=100, main = "Histogram of PON rate without the blocked bins", xlab = NULL)
pon.rate = median(pon$rate[!(pon$blocked | pon$rate == 0)])
#abline(v=c(pon.rate, pon.rate/2*1), col="red")
# does not show the pon variance

# binomial test with p adjustment according to the benjamin hochberg method 
pon$binom = apply(pon[, c("patient", "sum", "ploidy")], 1, function(x) ifelse(x[2] > 0, binom.test(x[1], x[2]+x[1], pon.rate/2*x[3])$p.value, 1))
pon$binom.adjust = p.adjust(pon$binom, method="hochberg")

#par(mar=c(2, 4, 1, 1))
#plot(pon$bin, pon$patient,
#     col = ifelse(!pon$blocked & pon$binom.adjust < 0.001, "#ff000011", "#11ff1111"),
...
lines(pat.dens, col="red")
legend("topright", legend=c("theoretical z-Distribution", "Patient", "scaled PON"), col=c("gray", "red", "blue"), lty=1, cex=0.7)
  
pon$gauss = apply(pon[, c("patient.norm", "norm.clean.mean", "norm.clean.sd")], 1, function(x) ifelse(x[1] == 0, 1, min(pnorm(q=x[1], mean=x[2], sd=x[3], lower.tail=x[1]<x[2]), 1)*2))
pon$gauss.adjust = p.adjust(pon$gauss, method="hochberg")

par(mar=c(2, 4, 1, 1))
with(pon[!pon$blocked, ],
     plot(bin, patient.norm*ploidy, ylim=c(0, 4),
          col = ifelse(!pon$blocked & pon$binom.adjust < 0.001, "#ff000011", "#11111111"),
          pch = 16, xaxt="n", xlab="", las = 1, ylab="ploidy", xaxs="i")
)
with(pon[!pon$blocked & pon$gauss.adjust < 0.01 , ],
     arrows(bin, norm.clean.mean*ploidy, bin, patient.norm*ploidy,
            length=0.03, angle=45, code=2, col="#ff000022", lwd=3))
abline(v = c(pon$bin[pon$Start == 1], nrow(pon)), col="#55555555", cex = 0.5)
with(contigs[(1:(nrow(contigs)/2))*2, ], axis(side=1, labels=chr, at=pos, line=-.5, cex.axis=0.5, tick=F))
with(contigs[(1:(nrow(contigs)/2))*2-1, ], axis(side=1, labels=chr, at=pos, line=-1, cex.axis=0.5, tick=F))
```
...
pon$aberration = with(pon[!pon$gc_blocked & !pon$var_blocked, ], factor(ifelse((pon$patient.norm > pon$norm.clean.mean & pon$binom.adjust < 0.05), 'amp', ifelse((pon$patient.norm < pon$norm.clean.mean & pon$binom.adjust < 0.05), 'del', 'normal')), levels = c('normal', 'amp', 'del')))
```
...
```{r segmentation algorithm, echo = FALSE}
  ...
  segment_start = 0
  for (i in 1:nrow(pon[!pon$blocked, ])) {
...
```{r genes in aberration bins table, echo = FALSE}
  ...
  g=0
  for (g in 1:nrow(pon[!pon$gc_blocked & !pon$var_blocked, ])) {
...
plot_bins_regions_ideogram <- function(pon, cytobands, segments, chrn, sd_factor) {
  ...
  y <- pon[pon$Chr == chrn, "patient.norm"] *pon[pon$Chr == chrn, "ploidy"]
  plot_patient <- qplot(x, y, ylim = c(-1, 4)) +
    
                  geom_line(aes(x = pon[pon$Chr == chrn, "Start"],
                                y = pon[pon$Chr == chrn, "norm.clean.mean"]
                                     *pon[pon$Chr == chrn, "ploidy"]),
                                color = "gray", linetype = "solid") +
    
                  geom_errorbar(aes(x = pon[pon$Chr == chrn, "Start"],
                                    ymin = pon[pon$Chr == chrn, "norm.clean.mean"]
                                           *pon[pon$Chr == chrn, "ploidy"] - 
                                    pon[pon$Chr == chrn, "norm.clean.sd"]
                                     *pon[pon$Chr == chrn, "ploidy"], 
                                    ymax = pon[pon$Chr == chrn, "norm.clean.mean"]
                                           *pon[pon$Chr == chrn, "ploidy"] + 
                                    pon[pon$Chr == chrn, "norm.clean.sd"]
                                     *pon[pon$Chr == chrn, "ploidy"]),
