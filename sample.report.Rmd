...
pon$aberration = factor(ifelse(pon$patient.norm > pon$norm.clean.median & pon$gauss.adjust < 0.05, 'amp', ifelse(pon$patient.norm < pon$norm.clean.median & pon$gauss.adjust < 0.05, 'del', 'normal')), levels = c('normal', 'amp', 'del'))
```
...
for(c in 1:nrow(contigs)){
  if(sum(pon$aberration[pon$Chr == contigs$chr[c]] == 'amp')/sum(pon$Chr == contigs$chr[c]) > 0.8) karyotype_to_report[(nrow(karyotype_to_report) +1), "karyotype"] = as.character(paste('chr', c, 'amp'))
  else if(sum(pon$aberration[pon$Chr == contigs$chr[c]] == 'del')/sum(pon$Chr == contigs$chr[c]) > 0.8) karyotype_to_report[(nrow(karyotype_to_report) +1), "karyotype"] = as.character(paste('chr', c, 'del'))
  # numerical chromosome changes
  
  else if(sum(pon$aberration[pon$Chr == contigs$chr[c] & !is.na(pon$arm) & pon$arm == 'p'] == 'amp')/sum(pon$Chr == contigs$chr[c] & !is.na(pon$arm) & pon$arm == 'p') > 0.8) karyotype_to_report[(nrow(karyotype_to_report) +1), "karyotype"] = as.character(paste(c, 'p amp'))
  else if(sum(pon$aberration[pon$Chr == contigs$chr[c] & !is.na(pon$arm) & pon$arm == 'p'] == 'del')/sum(pon$Chr == contigs$chr[c] & !is.na(pon$arm) & pon$arm == 'p') > 0.8) karyotype_to_report[(nrow(karyotype_to_report) +1), "karyotype"] = as.character(paste(c, 'p del'))
  else if(sum(pon$aberration[pon$Chr == contigs$chr[c] & !is.na(pon$arm) & pon$arm == 'q'] == 'amp')/sum(pon$Chr == contigs$chr[c]& !is.na(pon$arm) & pon$arm == 'q') > 0.8) karyotype_to_report[(nrow(karyotype_to_report) +1), "karyotype"] = as.character(paste(c, 'q amp'))
  else if(sum(pon$aberration[pon$Chr == contigs$chr[c] & !is.na(pon$arm) & pon$arm == 'q'] == 'del')/sum(pon$Chr == contigs$chr[c] & !is.na(pon$arm) & pon$arm == 'q') > 0.8) karyotype_to_report[(nrow(karyotype_to_report) +1), "karyotype"] = as.character(paste(c, 'q del'))
    # whole chromosome arms is del or amp
}
...
for(i in unique(pon$Chr)) {
  with(pon[pon$Chr == i,], {
    plot(Start, ifelse(patient.norm > 2.5, 5, patient.norm*2), col = ifelse(blocked, 0, c("black", "blue", "red")[aberration]), ylim=c(0,5), xlim=c(0,max(End)), xaxt='n', xaxs="i", xlab='', ylab='Ploidy', pch=ifelse(patient.norm > 2.5, 17, 20), las=2)
