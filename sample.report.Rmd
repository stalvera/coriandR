...
#knitr::opts_knit$set(root.dir = "/media/vera/big_data/coriandR/output/17828.munich.13.07/")
...
with(pon[!pon$blocked, ],
     plot(bin, patient.norm*2, ylim=c(0, 4),
          ...
          pch = 20, xaxt="n", xlab="", las = 1, ylab="ploidy", xaxs="i")
)
...
plot(pon$patient.norm/pon$norm.clean.mean, pon$gc, type='p',
     main="Correlation ratio patient/median pon to gc content", pch = 18,
     ylab="Ratio patient/median pon", xlab="gc content", xlim = c(0.3, 1))
```
...
for(i in unique(pon$Chr)) {
  with(pon[pon$Chr == i,], {
    plot(Start, ifelse(patient.norm > 2, 4, patient.norm*2), col = ifelse(blocked, 0, c("black", "blue", "red")[aberration]), ylim=c(0,4), xaxt='n', xlab='', ylab='Ploidy', pch=ifelse(patient.norm > 2, 17, 20), las=2)
    segments(Start, (norm.clean.median+norm.clean.sd*2)*2, Start, (norm.clean.median-norm.clean.sd*2)*2, col="#aaaaaaaa")
  ...
  legend("right", c("amplification", "deletion", "stalk", "centromer"), fill=c("white", "white", "#aaaa0055", "#00990055"), col=c("blue", "red", NA, NA), xpd=T, inset=c(-.13, 0), border=F, lty=0, pch=c(20, 20, NA, NA), cex=0.8)
}
```
